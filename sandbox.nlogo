...
; setup-nodes
;
; Initializes all turtles to the graph, creates as many turtles as
; NUMBER-OF-NODES. Sets the color of each node to blue, and positions
; each node so that there is a .05 buffer from the edge of the screen.
; The shape of each turtle is set to a circle.
;
...
; setup-links
;
; Initializes links to and from each node, generating a network from
; the resulting connections.  This is done by "asking" each node to
